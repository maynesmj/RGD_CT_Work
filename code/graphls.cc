...
*/	
	TFile f("graph_lc_3D.root", "update");
	...
	auto pbw_g = new TGraph();
      ...
      double ccon [5] = { 367.595, 146.619, 48.2844, 13.8532,2.44704};
      double cmean [5] = { 0.77144, 0.774394, 0.778713, 0.784104,0.788976};
      
      double dcon [5] = { 460.093, 172.829, 59.3194, 17.8224,3.72275};
      double dmean [5] = { 0.770876, 0.772275, 0.77407, 0.780049,0.792714};
      ...
      for(int i = 0; i < 5; i++){
         ...
         for(int j = 0; j <= 100000000; j++){
            yc = yc + ccon[i]*(1/(2*3.14159))*(0.15/(4*((cmean[i] - xs)*(cmean[i] - xs)) + (0.15*0.15)));
            yd = yd + dcon[i]*(1/(2*3.14159))*(0.15/(4*((dmean[i] - xs)*(dmean[i] - xs)) + (0.15*0.15)));
            xs = xs + 0.000000004;
          }
      
         ta[i] = (yc * 0.82) / (yd * 0.88);
