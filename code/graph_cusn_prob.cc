...
void graph_cusn_better(){
	...
	TFile f("aaaaaaaaaaagraph_cxc_3D_aps_9.root", "update");
   ...
   */
   ...
   double sn7e = 110102700;
    ...
    //      std::cout << "ybwy = " << ybwy/0.01 << std::endl;
         ...
         int ycc = 1.9;
         int ydc = 4.12;
         ta[i] = ((cxc0v77_yields[i] * 0.82 ) / (ld0v77_yields[i] * 0.88)) ;//* ((27.5931)/(26.0234));  //(ld07_e[i]/cxc07_e[i]);       //0.0833 for cu   ccon[i] dcon[i]
         ta2[i] = ((cu0v77_yields[i] * 0.82) / (ld0v77_yields[i] * 0.0833 ) ) ;//* ((27.5931)/(157.914));//(ld07_e[i]/(cu07_e[i] + sn07_e[i]));    // * (cu7e/l7e);       //0.0833 for cu   * (l7e/cu7e)
         ta3[i] = ((sn0v77_yields[i] * 0.82 ) / (ld0v77_yields[i] * 0.125 ) ) ;//* ((27.5931)/(157.914));//(ld07_e[i]/(cu07_e[i] + sn07_e[i]));      //* (sn7e/l7e);          //ddcon
         ...
         eta[i] = (ta[i] * sqrt((1/cxc0v77_yields[i])+ (1/ld0v77_yields[i]))) / 2;
         eta2[i] = (ta2[i] * sqrt((1/cu0v77_yields[i])+ (1/ld0v77_yields[i]))) / 2;
         eta3[i] = (ta3[i] * sqrt((1/sn0v77_yields[i])+ (1/ld0v77_yields[i]))) / 2;
      ...
      nt4->SetMarkerStyle(21);
        ...
        nt->SetTitle("APS Carbon Nuclear Transparency;Q^{2} [GeV^{2}];T_{A}");
        nt->SetMinimum(0.2);
   	nt->SetMaximum(1.1);
   	...
   	nt2->SetTitle("Copper Nuclear Transparency;Q^{2} [GeV^{2}];T_{A}");
   	nt2->SetMinimum(0.2);
   	nt2->SetMaximum(1.1);
   	nt2->Draw("AP");
   	nt2->Write();
   	
   	nt3->SetTitle("Tin Nuclear Transparency;Q^{2} [GeV^{2}];T_{A}");
   	nt3->SetMinimum(0.2);
   	nt3->SetMaximum(1.1);
   	nt3->Draw("AP");
   	nt3->Write();
