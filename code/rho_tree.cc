...
int main() {                   //int argc char** argv
   ...
   TH1F *cuq_hist = new TH1F("Cu Q2", "Cu Q2", 300, -20, 20);
   TH1F *cuw_hist = new TH1F("Cu w", "Cu w", 300, -20, 20);
   ...
   TFile *input = new TFile("cusn_skimp_pass0v4_300.root", "read");

   TFile *output = new TFile( "Cu_Sn_pass0v4_300_tree_2.root", "recreate");

   TTree *tree = (TTree*)input->Get("electron_tree");
   TTree *piptree = (TTree*)input->Get("pip_tree");
   TTree *pimtree = (TTree*)input->Get("pim_tree");
   //TFile f("cusn_18624_hist_10.root", "update");
...
std::vector<float> sn_vx;
   ...
   cu_tree->Branch("phi", &phi, "phi/F");
   cu_tree->Branch("px", &px, "px/F");
   cu_tree->Branch("py", &py, "py/F");
   cu_tree->Branch("pz", &pz, "pz/F");
   //cu_tree->Branch("vx", &vx, "vx/F");
   //cu_tree->Branch("vy", &vy, "vy/F");
   cu_tree->Branch("vz", &vz, "vz/F");
   cu_tree->Branch("energy", &energy, "energy/F");
   
   
   //cu_pip_tree->Branch("npip", &npip, "npip/I");
   cu_pip_tree->Branch("a_pip_px", a_pip_px, TString::Format("a_pip_px[%d]/F", 10));
   cu_pip_tree->Branch("a_pip_py", a_pip_py, TString::Format("a_pip_py[%d]/F", 10));
   cu_pip_tree->Branch("a_pip_pz", a_pip_pz, TString::Format("a_pip_pz[%d]/F", 10));
   //cu_pip_tree->Branch("a_pip_vx", a_pip_vx, TString::Format("a_pip_vx[%d]/F", 10));
   //cu_pip_tree->Branch("a_pip_vy", a_pip_vy, TString::Format("a_pip_vy[%d]/F", 10));
   //cu_pip_tree->Branch("a_pip_vz", a_pip_vz, TString::Format("a_pip_vz[%d]/F", 10));
   //cu_pip_tree->Branch("a_pip_vt", a_pip_vt, TString::Format("a_pip_vt[%d]/F", 10));
   //cu_pip_tree->Branch("a_pip_beta", a_pip_beta, TString::Format("a_pip_beta[%d]/F", 10));
   cu_pip_tree->Branch("a_pip_energy", a_pip_energy, TString::Format("a_pip_energy[%d]/F", 10));

   //cu_pim_tree->Branch("npim", &npim, "npim/I");
   cu_pim_tree->Branch("a_pim_px", a_pim_px, TString::Format("a_pim_px[%d]/F", 10));
   cu_pim_tree->Branch("a_pim_py", a_pim_py, TString::Format("a_pim_py[%d]/F", 10));
   cu_pim_tree->Branch("a_pim_pz", a_pim_pz, TString::Format("a_pim_pz[%d]/F", 10));
   //cu_pim_tree->Branch("a_pim_vx", a_pim_vx, TString::Format("a_pim_vx[%d]/F", 10));
   //cu_pim_tree->Branch("a_pim_vy", a_pim_vy, TString::Format("a_pim_vy[%d]/F", 10));
   //cu_pim_tree->Branch("a_pim_vz", a_pim_vz, TString::Format("a_pim_vz[%d]/F", 10));
   //cu_pim_tree->Branch("a_pim_vt", a_pim_vt, TString::Format("a_pim_vt[%d]/F", 10));
   //cu_pim_tree->Branch("a_pim_beta", a_pim_beta, TString::Format("a_pim_beta[%d]/F", 10));
   cu_pim_tree->Branch("a_pim_energy", a_pim_energy, TString::Format("a_pim_energy[%d]/F", 10));
   ...
   sn_tree->Branch("phi", &phi, "phi/F");
   sn_tree->Branch("px", &px, "px/F");
   sn_tree->Branch("py", &py, "py/F");
   sn_tree->Branch("pz", &pz, "pz/F");
   //sn_tree->Branch("vx", &vx, "vx/F");
   //sn_tree->Branch("vy", &vy, "vy/F");
   sn_tree->Branch("vz", &vz, "vz/F");
   //sn_tree->Branch("vt", &vt, "vt/F");
   //sn_tree->Branch("beta", &beta, "beta/F");
   sn_tree->Branch("energy", &energy, "energy/F");
   //tree->Branch("Q2", &Q2, "Q2/F");
   //tree->Branch("w", &w, "w/F");

   //sn_pip_tree->Branch("npip", &npip, "npip/I");
   sn_pip_tree->Branch("a_pip_px", a_pip_px, TString::Format("a_pip_px[%d]/F", 10));
   sn_pip_tree->Branch("a_pip_py", a_pip_py, TString::Format("a_pip_py[%d]/F", 10));
   sn_pip_tree->Branch("a_pip_pz", a_pip_pz, TString::Format("a_pip_pz[%d]/F", 10));
   //sn_pip_tree->Branch("a_pip_vx", a_pip_vx, TString::Format("a_pip_vx[%d]/F", 10));
   //sn_pip_tree->Branch("a_pip_vy", a_pip_vy, TString::Format("a_pip_vy[%d]/F", 10));
   //sn_pip_tree->Branch("a_pip_vz", a_pip_vz, TString::Format("a_pip_vz[%d]/F", 10));
   //sn_pip_tree->Branch("a_pip_vt", a_pip_vt, TString::Format("a_pip_vt[%d]/F", 10));
   //sn_pip_tree->Branch("a_pip_beta", a_pip_beta, TString::Format("a_pip_beta[%d]/F", 10));
   sn_pip_tree->Branch("a_pip_energy", a_pip_energy, TString::Format("a_pip_energy[%d]/F", 10));

   //sn_pim_tree->Branch("npim", &npim, "npim/I");
   sn_pim_tree->Branch("a_pim_px", a_pim_px, TString::Format("a_pim_px[%d]/F", 10));
   sn_pim_tree->Branch("a_pim_py", a_pim_py, TString::Format("a_pim_py[%d]/F", 10));
   sn_pim_tree->Branch("a_pim_pz", a_pim_pz, TString::Format("a_pim_pz[%d]/F", 10));
   //sn_pim_tree->Branch("a_pim_vx", a_pim_vx, TString::Format("a_pim_vx[%d]/F", 10));
   //sn_pim_tree->Branch("a_pim_vy", a_pim_vy, TString::Format("a_pim_vy[%d]/F", 10));
   //sn_pim_tree->Branch("a_pim_vz", a_pim_vz, TString::Format("a_pim_vz[%d]/F", 10));
   //sn_pim_tree->Branch("a_pim_vt", a_pim_vt, TString::Format("a_pim_vt[%d]/F", 10));
   //sn_pim_tree->Branch("a_pim_beta", a_pim_beta, TString::Format("a_pim_beta[%d]/F", 10));
   sn_pim_tree->Branch("a_pim_energy", a_pim_energy, TString::Format("a_pim_energy[%d]/F", 10));
   int mul = 3;
   float snl = (-2.59393) - mul*(0.530068);
   float snu = (-2.59393) + mul*(0.530068);//-1.438985;
   float cul = (-7.52991) - mul*(0.528162);//-10.767928;
   float cuu = (-7.52991) + mul*(0.528162);//-6.246532;   // num - 0.5
   
   float sn1_lower_bound = snl; //(-(pow(0.722434,3)) + (-3.179));
   float sn1_upper_bound = snu; //((pow(0.722434,3)) + (-3.179));
   
   float cu1_lower_bound = cul; //(-(pow(0.7668,3)) + (-8.104));
   float cu1_upper_bound = cuu; //((pow(0.7668,3)) + (-8.104));

   float sn2_lower_bound = snl; //(-(pow(0.80906,3)) + (-3.039));  //prior to alignment and calibration mean = -3.76856 and the sigma = 0.992765
   float sn2_upper_bound = snu; //((pow(0.80906,3)) + (-3.039));
   
   float cu2_lower_bound = cul; //(-(pow(0.809467,3)) + (-7.979));   // mean = -8.68684 and sigma = 1.04883
   float cu2_upper_bound = cuu; //((pow(0.809467,3)) + (-7.979));


   float sn3_lower_bound = snl; //(-(pow(0.783339,3)) + (-3.51069));
   float sn3_upper_bound = snu; //((pow(0.783339,3)) + (-3.51069));
   
   float cu3_lower_bound = cul; //(-(pow(0.804589,3)) + (-8.41561));
   float cu3_upper_bound = cuu; //((pow(0.804589,3)) + (-8.41561));


   float sn4_lower_bound = snl; //(-(pow(0.727373,3)) + (-3.111));
   float sn4_upper_bound = snu; //((pow(0.727373,3)) + (-3.111));
   
   float cu4_lower_bound = cul; //(-(pow(0.769119,3)) + (-8.064));
   float cu4_upper_bound = cuu; //((pow(0.769119,3)) + (-8.064));


   float sn5_lower_bound = snl; //(-(pow(0.640188,3)) + (-2.37849));
   float sn5_upper_bound = snu; //((pow(0.640188,3)) + (-2.37849));
   
   float cu5_lower_bound = cul; //(-(pow(0.683245,3)) + (-7.335));
   float cu5_upper_bound = cuu; //((pow(0.683245,3)) + (-7.335));


   float sn6_lower_bound = snl; //(-(pow(0.688119,3)) + (-2.74902));
   float sn6_upper_bound = snu; //((pow(0.688119,3)) + (-2.74902));
   
   float cu6_lower_bound = cul; //(-(pow(0.743513,3)) + (-7.703));
   float cu6_upper_bound = cuu; //((pow(0.743513,3)) + (-7.703));
      ...
      for(int i = 0; i < nentries; i++){
         ...
         std::cout << i << "|" << nentries << std::endl;
            ...
            float sec_dec =  phi; //el_lv.Phi() * 180/M_PI;  //atan(sqrt(pow(px,2) + pow(py,2))/pz);// * 180/M_PI;
				...
				ivz_hist->Fill(vz);
				     if((cu1_lower_bound) <= vz && (cu1_upper_bound) >= vz){			      
    						cu_vx.push_back(vx);
   						cu_vy.push_back(vy);
   						cu_vz.push_back(vz);
   						cu_px.push_back(px);
   						...
   						cu_energy.push_back(energy);
                                            ...
                                            cu_pim_tree->Fill();
                                              cu_count++;
				     } else if ((sn1_lower_bound) <= vz && (sn1_upper_bound) >= vz){
    						sn_vx.push_back(vx);
   						sn_vy.push_back(vy);
   						sn_vz.push_back(vz);
   						sn_px.push_back(px);
				...
				iivz_hist->Fill(vz);
				if((cu2_lower_bound) <= vz && (cu2_upper_bound) >= vz){			      
    						cu_vx.push_back(vx);
   						cu_vy.push_back(vy);
   						cu_vz.push_back(vz);
   						cu_px.push_back(px);
   						...
   						cu_energy.push_back(energy);
   						  ...
   						  cu_tree->Fill();
                                            ...
                                            cu_pim_tree->Fill();
                                              cu_count++;
				     } else if ((sn2_lower_bound) <= vz && (sn2_upper_bound) >= vz){
    						sn_vx.push_back(vx);
   						sn_vy.push_back(vy);
   						sn_vz.push_back(vz);
   						sn_px.push_back(px);
				...
				iiivz_hist->Fill(vz);
				if((cu3_lower_bound) <= vz && (cu3_upper_bound) >= vz){			      
    						cu_vx.push_back(vx);
   						cu_vy.push_back(vy);
   						cu_vz.push_back(vz);
   						cu_px.push_back(px);
   						...
   						cu_energy.push_back(energy);
   						 ...
   						 cu_tree->Fill();
                                            ...
                                            cu_pim_tree->Fill();
                                           cu_count++;
				     } else if ((sn3_lower_bound) <= vz && (sn3_upper_bound) >= vz){
    						sn_vx.push_back(vx);
   						sn_vy.push_back(vy);
   						sn_vz.push_back(vz);
   						sn_px.push_back(px);
				...
				ivvz_hist->Fill(vz);
				if((cu4_lower_bound) <= vz && (cu4_upper_bound) >= vz){			      
    						cu_vx.push_back(vx);
   						cu_vy.push_back(vy);
   						cu_vz.push_back(vz);
   						cu_px.push_back(px);
   						...
   						cu_energy.push_back(energy);
   						  ...
   						  cu_tree->Fill();
                                            ...
                                            cu_pim_tree->Fill();
                                               cu_count++;
				     } else if ((sn4_lower_bound) <= vz && (sn4_upper_bound) >= vz){
    						sn_vx.push_back(vx);
   						sn_vy.push_back(vy);
   						sn_vz.push_back(vz);
   						sn_px.push_back(px);
				...
				vvz_hist->Fill(vz);
				if((cu5_lower_bound) <= vz && (cu5_upper_bound) >= vz){			      
    						cu_vx.push_back(vx);
   						cu_vy.push_back(vy);
   						cu_vz.push_back(vz);
   						cu_px.push_back(px);
   						...
   						cu_energy.push_back(energy);
   						  ...
   						  cu_tree->Fill();
                                            ...
                                            cu_pim_tree->Fill();
                                               cu_count++;
				     } else if ((sn5_lower_bound) <= vz && (sn5_upper_bound) >= vz){
    						sn_vx.push_back(vx);
   						sn_vy.push_back(vy);
   						sn_vz.push_back(vz);
   						sn_px.push_back(px);
				...
				vivz_hist->Fill(vz);
				if((cu6_lower_bound) <= vz && (cu6_upper_bound) >= vz){			      
    						cu_vx.push_back(vx);
   						cu_vy.push_back(vy);
   						cu_vz.push_back(vz);
   						cu_px.push_back(px);
				     ...
				     } else if ((sn6_lower_bound) <= vz && (sn6_upper_bound) >= vz){
    						sn_vx.push_back(vx);
   						sn_vy.push_back(vy);
   						sn_vz.push_back(vz);
   						sn_px.push_back(px);
