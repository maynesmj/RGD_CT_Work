...
TH1F *pimwtzp_hist = new TH1F("#Pi^{-} Momentum With W, -t & Zh cut", "#Pi^{-} Momentum With W, -t & Zh cut", 200, 0, 10);
   ...
   TH1F *ivp_hist = new TH1F("Q2 (3.5->6)", "Q2 (3.5->6)", 200, 0.2, 2);
...
//9110, 9111, 9112, 9113, 9114, 9115, 9117, 9118, 9119
   ...
   TFile f("all_pass0v11_zzgg_obnt_32.root", "update"); //cusn_pass0v7_63_test.root

    std::ifstream v3("all_pass0v11_cskim.list");  //   rge_align.list    cusn_calv9_skim.list   cxc_pass0v10_cskim.list   ld_pass0v10_cskim.list    cusn_pass0v10_cskim.list
    std::ifstream v4("all_pass0v11_run.list");
    std::ifstream v5("all_pass0v11_charge.list");
    std::ifstream v6("all_pass0v11_fn.list");
         std::string line;
...
//std::cout << "1 th" << std::endl;
 ...
 int e_num = 0;
  ...
  double nt_charge = 0.0;
                       ...
                       int end_p =  v3_a.size();
      for(int p = 0; p < end_p; p++){ // for(int p = 0; p < v3_a.size(); p++){  cusn(34 37)33  cxc(18, 20)13  LD2(18,)
        std::string inFile;
            inFile =  v3_a[p];
   ...
   std::vector<std::string> eve_bC;
     ...
     // hipo::bank fmt(factory.getSchema("FMT::Tracks"));
     ...
     /*
      ...
      /*
        ...
        */ 
         ...
         //double pimz_ucut = -5.74827;
         //double pimz_lcut = -6.29121;  //for Tin
         //double pimz_ucut = 5;
...
//inbending
         ...
         //double pipz_ucut = -5.26301;
         //double pipz_lcut = -6.15124;  //for Tin
         //double pipz_ucut = 5;
         //double pipz_lcut = -15;  //for Liquid Deuterium
         //double pipz_ucut = 5;
     ...
     //    std::cout << row << " | " << nrows << " | " << ((p + 1.0) / v3_a.size()) * 100 << "%" <<std::endl;
         ...
         // }
            ...
            if(chi2 > -10 && chi2 < 10 && temp_vz >= pipz_lcut && temp_vz <= pipz_ucut ){ //&& abs(temp_s)/2000 == 1){	
            ...
            if(chi2 > -10 && chi2 < 10 && temp_vz >= pimz_lcut && temp_vz <= pimz_ucut ){ //&& abs(temp_s)/2000 == 1){	
 ...
 //         tvz6_fit->SetParNames("enter const", "enter mean", "enter sigma", "exit const", "exit mean", "exit sigma");
  ...
  tvz6_hist->GetXaxis()->SetTitle(std::to_string(v4_a[p]).c_str());    
   ...
   tvz6_g->AddPoint(v4_a[p], tvz62_fit->GetParameter(4));
      ...
      std::cout << "p = " << ((p + 1.0) / end_p) * 100 << "%" <<std::endl;
