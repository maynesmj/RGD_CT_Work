...
int main(){
   ...
   TH1D *q_hist = new TH1D("lc", " lc ", 200, 0, 10);
   ...
   TH1D *ivp_hist = new TH1D("sector 4 P", "Q2 (3.5->4)", 200, 0, 2);
  ...
  // std::cout << "1 th" << std::endl;
   ...
   TFile f("cxc_ob_kin_18459.root", "update");

    std::ifstream v3("cxc_ob_18459.list");
         std::string line;
   ...
   std::vector<std::string> eve_bC;
      ...
      while(reader.next() == true){
       ...
       //      e_total = e_total + pcal;
             ...
             if(cher.getInt("detector", row) == 15){
                 //float pcal = cal.getFloat("energy", row);
		        float nphe = cher.getFloat("nphe", row);
             }
                  //e_total = e_total + pcal;
		//        wonphe_hist->Fill(nphe);
		//        //wopcal_hist->Fill(pcal);
		//        wocut_hist->Fill(nphe, pcal);
